...
class InterceptResolver(BaseResolver):
    ...
    def __init__(
        self, address, port, ttl, intercept, skip, nxdomain, forward, all_qtypes, timeout=0
    ):
        """
        ...
        if not reply.rr:
            try:
                if handler.protocol == "udp":
                    proxy_r = request.send(upstream, upstream_port, timeout=self.timeout)
                else:
                    proxy_r = request.send(upstream, upstream_port, tcp=True, timeout=self.timeout)
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--nxdomain", "-x", action="append", metavar="<label>", help="Return NXDOMAIN (glob)"
    )
    p.add_argument(
        "--forward",
        "-f",
    ...
    p.add_argument(
        "--ttl", "-t", default="60s", metavar="<ttl>", help="Intercept TTL (default: 60s)"
    )
    p.add_argument(
        "--timeout",
        "-o",
