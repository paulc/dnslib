...
class ShellResolver(BaseResolver):
    ...
    def resolve(self, request, handler):
        ...
        if cmd:
            ...
            reply.add_answer(RR(qname, QTYPE.TXT, ttl=self.ttl, rdata=TXT(output[:254])))
        else:
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--origin", "-o", default=".", metavar="<origin>", help="Origin domain label (default: .)"
    )
    p.add_argument(
        "--ttl", "-t", default="60s", metavar="<ttl>", help="Response TTL (default: 60s)"
    )
    p.add_argument(
        "--port", "-p", type=int, default=53, metavar="<port>", help="Server port (default:53)"
    )
    p.add_argument(
        "--address", "-a", default="", metavar="<address>", help="Listen address (default:all)"
    )
    p.add_argument(
        "--udplen",
        "-u",
    ...
    p.add_argument(
        "--tcp", action="store_true", default=False, help="TCP server (default: UDP only)"
    )
    p.add_argument(
        "--log",
