...
class DNSRecord(object):
    ...
    def parse(cls, packet):
        ...
        try:
            ...
            raise DNSError(
                "Error unpacking DNSRecord [offset=%d]: %s"
                % (
                    buffer.offset,
                    e,
                ),
    ...
    def reply_zone(self, zone, ra=1, aa=1):
        """
        Create reply with response data in zone-file format
        >>> q = DNSRecord.question("abc.com")
        >>> a = q.reply_zone("abc.com 60 A 1.2.3.4")
        >>> print(a)
        ;; ->>HEADER<<- opcode: QUERY, status: NOERROR, id: ...
        ;; flags: qr aa rd ra; QUERY: 1, ANSWER: 1, AUTHORITY: 0, ADDITIONAL: 0
        ;; QUESTION SECTION:
    ...
    def add_answer(self, *rr):
        ...
        >>> q = DNSRecord.question("abc.com")
        >>> a = q.reply()
        >>> a.add_answer(*RR.from_zone("abc.com A 1.2.3.4"))
        >>> print(a)
        ;; ->>HEADER<<- opcode: QUERY, status: NOERROR, id: ...
        ;; flags: qr aa rd ra; QUERY: 1, ANSWER: 1, AUTHORITY: 0, ADDITIONAL: 0
        ;; QUESTION SECTION:
    ...
    def add_auth(self, *auth):
        ...
        >>> a.add_auth(*RR.from_zone("abc.com 3600 NS nsa.abc.com"))
        >>> print(a)
        ;; ->>HEADER<<- opcode: QUERY, status: NOERROR, id: ...
    ...
    def add_ar(self, *ar):
        ...
        >>> q = DNSRecord.question("abc.com")
        >>> a = q.reply()
        >>> a.add_answer(*RR.from_zone("abc.com 60 CNAME x.abc.com"))
        >>> a.add_ar(*RR.from_zone("x.abc.com 3600 A 1.2.3.4"))
        >>> print(a)
        ;; ->>HEADER<<- opcode: QUERY, status: NOERROR, id: ...
    ...
    def pack(self):
        ...
        >>> q = DNSRecord.question("abc.com")
        >>> q.header.id = 1234
        >>> a = q.reply_zone("abc.com A 1.2.3.4")
        >>> a.header.aa = 0
        >>> pkt = a.pack()
    ...
    def truncate(self):
        ...
        >>> q = DNSRecord.question("abc.com")
        >>> a = q.reply()
        >>> a.add_answer(*RR.from_zone('abc.com IN TXT %s' % ('x' * 255)))
        >>> a.add_answer(*RR.from_zone('abc.com IN TXT %s' % ('x' * 255)))
        >>> a.add_answer(*RR.from_zone('abc.com IN TXT %s' % ('x' * 255)))
        >>> len(a.pack())
        829
    ...
    def to_zone(self, prefix=""):
        """
        Formatted 'DiG' (zone) style output
        (with optional prefix)
        """
        z = self.header.to_zone().split("\n")
        if self.questions:
            z.append(";; QUESTION SECTION:")
            [z.extend(q.to_zone().split("\n")) for q in self.questions]
        if self.rr:
            z.append(";; ANSWER SECTION:")
            [z.extend(rr.to_zone().split("\n")) for rr in self.rr]
        if self.auth:
    ...
    def short(self):
        ...
        Just return RDATA
        """
        return "\n".join([rr.rdata.to_zone() for rr in self.rr])
...
class DNSHeader(object):
    ...
    def parse(cls, buffer):
        ...
        try:
            ...
            raise DNSError(
                "Error unpacking DNSHeader [offset=%d]: %s"
                % (
                    buffer.offset,
                    e,
                ),
...
class DNSQuestion(object):
    ...
    def parse(cls, buffer):
        try:
            ...
            raise DNSError(
                "Error unpacking DNSQuestion [offset=%d]: %s"
                % (
                    buffer.offset,
                    e,
                ),
...
class RR(object):
    ...
    @classmethod
    def from_zone(cls, zone, origin="", ttl=0):
        """
...
class EDNS0(RR):
    ...
    >>> r = DNSRecord.question("abc.com").reply_zone("abc.com A 1.2.3.4")
    >>> r.add_ar(opt)
    >>> print(r)
    ;; ->>HEADER<<- opcode: QUERY, status: NOERROR, id: ...
    ;; flags: qr aa rd ra; QUERY: 1, ANSWER: 1, AUTHORITY: 0, ADDITIONAL: 1
...
class RD(object):
    ...
    def parse(cls, buffer, length):
        ...
        try:
            ...
            raise DNSError(
                "Error unpacking RD [offset=%d]: %s" % (buffer.offset, e),
            )
...
class TXT(RD):
    ...
    "txtvers=1","swver=2.5"
    >>> a = DNSRecord()
    >>> a.add_answer(*RR.from_zone('example.com 60 IN TXT "txtvers=1"'))
    >>> a.add_answer(*RR.from_zone('example.com 120 IN TXT "txtvers=1" "swver=2.3"'))
    >>> print(a)
    ;; ->>HEADER<<- opcode: QUERY, status: NOERROR, id: ...
    ...
    def parse(cls, buffer, length):
        try:
            ...
            raise DNSError(
                "Error unpacking TXT [offset=%d]: %s" % (buffer.offset, e),
            )
...
class A(RD):
    ...
    def parse(cls, buffer, length):
        try:
            ...
            raise DNSError(
                "Error unpacking A [offset=%d]: %s" % (buffer.offset, e),
            )
...
class AAAA(RD):
    ...
    def parse(cls, buffer, length):
        try:
            ...
            raise DNSError(
                "Error unpacking AAAA [offset=%d]: %s" % (buffer.offset, e),
            )
...
class MX(RD):
    ...
    def parse(cls, buffer, length):
        try:
            ...
            raise DNSError(
                "Error unpacking MX [offset=%d]: %s" % (buffer.offset, e),
            )
...
class CNAME(RD):
    ...
    def parse(cls, buffer, length):
        try:
            ...
            raise DNSError(
                "Error unpacking CNAME [offset=%d]: %s" % (buffer.offset, e),
            )
...
class SOA(RD):
    ...
    def parse(cls, buffer, length):
        try:
            ...
            raise DNSError(
                "Error unpacking SOA [offset=%d]: %s" % (buffer.offset, e),
            )
...
class SRV(RD):
    ...
    def parse(cls, buffer, length):
        try:
            ...
            raise DNSError(
                "Error unpacking SRV [offset=%d]: %s" % (buffer.offset, e),
            )
...
class NAPTR(RD):
    ...
    def parse(cls, buffer, length):
        try:
            ...
            raise DNSError(
                "Error unpacking NAPTR [offset=%d]: %s" % (buffer.offset, e),
            )
...
class DS(RD):
    ...
    def parse(cls, buffer, length):
        try:
            ...
            raise DNSError(
                "Error unpacking DS [offset=%d]: %s" % (buffer.offset, e),
            )
...
class NSEC(RD):
    ...
    def parse(cls, buffer, length):
        try:
            ...
            raise DNSError(
                "Error unpacking NSEC [offset=%d]: %s" % (buffer.offset, e),
            )
...
class CAA(RD):
    ...
    >>> a = DNSRecord()
    >>> a.add_answer(*RR.from_zone('example.com 60 IN CAA 0 issue "letsencrypt.org"'))
    >>> print(a)
    ;; ->>HEADER<<- opcode: QUERY, status: NOERROR, id: ...
    ...
    def parse(cls, buffer, length):
        try:
            ...
            raise DNSError(
                "Error unpacking CAA [offset=%d]: %s" % (buffer.offset, e),
            )
...
class HTTPS(RD):
    ...
    >>> HTTPS.from_zone(["1", "cloudflare.com."])
    1 cloudflare.com.
    >>> HTTPS.from_zone(["1", ".", "mandatory=key65444,echconfig"])
    1 . mandatory=key65444,echconfig
    >>> HTTPS.from_zone(["1", ".", "alpn=h3,h3-29,h2"])
    1 . alpn=h3,h3-29,h2
    >>> HTTPS.from_zone(["1", ".", "no-default-alpn"])
    1 . no-default-alpn
    >>> HTTPS.from_zone(["1", ".", "port=443"])
    1 . port=443
    >>> HTTPS.from_zone(["1", ".", "ipv4hint=104.16.132.229,104.16.133.229"])
    1 . ipv4hint=104.16.132.229,104.16.133.229
    >>> HTTPS.from_zone(["1", ".", "echconfig=Z2FyYmFnZQ=="])
    1 . echconfig=Z2FyYmFnZQ==
    >>> HTTPS.from_zone(["1", ".", "ipv6hint=2606:4700::6810:84e5,2606:4700::6810:85e5"])
    1 . ipv6hint=2606:4700::6810:84e5,2606:4700::6810:85e5
    >>> HTTPS.from_zone(["1", ".", "key9999=X"])
    1 . key9999=X
...
class LOC(RD):
    ...
    >>> a.add_answer(*RR.from_zone('area51.local. 60 IN LOC 37 14 12.094 N 115 48 14.649 W 1381.00m'))
    >>> a.add_answer(*RR.from_zone('area51.local. 60 IN LOC 37 N 115 48 W 1381.00m'))
    >>> a.add_answer(*RR.from_zone('area51.local. 60 IN LOC 37 14 12.094 N 115 48 14.649 W 1381.00m 1m 10000m 10m'))
    >>> print(a)
    ...
    def parse(cls, buffer, length):
        try:
            ...
            raise DNSError(
                "Error unpacking LOC [offset=%d]: %s" % (buffer.offset, e),
            )
...
class RP(RD):
    ...
    def parse(cls, buffer, length):
        try:
            ...
            raise DNSError(
                "Error unpacking RP [offset=%d]: %s" % (buffer.offset, e),
            )
...
class ZoneParser:
    ...
    def parse_rr(self, rr):
        ...
        return RR(
            ...
            rdata=rd.from_zone(rdata, self.origin),
        )
